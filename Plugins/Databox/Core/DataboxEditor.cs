...
namespace Databox.Ed
{
	...
	public class DataboxEditor {
		...
		static Texture2D iconTrashIcon;
		static Texture2D iconReset;
		#endif
		...
		static bool showLocalization;
		static bool showSavePath;
		static bool showSerializer;
		static bool showEditorSettings;
		
		
		static int selectedAddValueOption = 0;
			...
			if (configOpen)
			{
				...
				using (new GUILayout.VerticalScope("Window"))
				{
					...
					if (showLocalization)
					{
						using (new GUILayout.VerticalScope("Box"))
						{
							GUILayout.Label("Databox Localization", "boldLabel");		
							
							EditorGUILayout.HelpBox("If you are using this Databox object as a Databox Localization table please set this to true.", MessageType.Info);
						
							database.isLocalizationTable = EditorGUILayout.Toggle("Is localization table:", database.isLocalizationTable);
								
								
							if (database.isLocalizationTable)
							{			
								if (GUILayout.Button("Setup languages table"))
								{	
									if (string.IsNullOrEmpty(database.fileName))
									{
										Debug.LogWarning("Databox: Please set a filename and save path");	
									}
									else
									{
										database.AddDatabaseTable("Languages");
										...
										database.AddData("Languages", "Languages", "Languages", System.Activator.CreateInstance( _foundType ) as DataboxType);
									}
								}
					...
					using (new GUILayout.HorizontalScope("Button"))
					{
						GUILayout.Space(15);
						#if UNITY_EDITOR
						showSavePath = EditorGUILayout.Foldout(showSavePath, "Save path", true);
						#else
						showSavePath = GUILayout.Toggle(showSavePath, "Save path");
						#endif
					...
					if (showSavePath)
					{
						using (new GUILayout.VerticalScope("Box"))
						{
							...
							switch(database.selectedApplicationPath)
							{
								...
								case 2:
									#if UNITY_EDITOR
									EditorGUILayout.HelpBox("Read only, use it to load data during runtime. Saving to a Resources folder only works inside the Unity editor.", MessageType.Warning);
						...
						#if UNITY_EDITOR
							if (database.selectedApplicationPath < 3)
							{
								if (GUILayout.Button("Show in explorer"))
								{
									if (System.IO.File.Exists(database.savePath))
									{
										if (database.selectedApplicationPath < 2)
										{
											var _explorerPath = database.savePath.Replace(@"/", @"\");   // explorer doesn't like front slashes
											EditorUtility.RevealInFinder(_explorerPath);
										}
										else
										{
											EditorUtility.RevealInFinder(Application.dataPath + "/" + database.savePath);
										}
									}
									else
									{
										...
										if (database.selectedApplicationPath < 2)
										{
											EditorUtility.RevealInFinder(System.IO.Path.GetDirectoryName(database.savePath));
										}
										else
										{
											EditorUtility.RevealInFinder(Application.dataPath + "/" + System.IO.Path.GetDirectoryName(database.savePath));
										}
									}
								}
							}
					...
					{
						GUILayout.Space(15);
						showSerializer = EditorGUILayout.Foldout(showSerializer, "Serializer", true);
					}
					...
					{
						GUILayout.Space(15);
						showEditorSettings = EditorGUILayout.Foldout(showEditorSettings, "Editor", true);
					}
					...
					if (showEditorSettings)
					{
						using (new GUILayout.VerticalScope("Box"))
						{
							GUILayout.Label("Automatic Save and Load (only in editor and when game is not running)");
							database.automaticSave = GUILayout.Toggle(database.automaticSave, "Automatic save");	
							database.automaticLoad = GUILayout.Toggle(database.automaticLoad, "Automatic load");	
						}
		...
		{
			#if UNITY_EDITOR
			if ((database.DB == null || database.DB.Keys.Count == 0) && !string.IsNullOrEmpty(database.fileName))
			{
				if (!database.automaticLoad)
				{
					EditorGUILayout.HelpBox("Automatic loading in editor is disabled. If enabled, Databox loads the file as soon as the databox object is selected. Please note that you will still have to load the database at runtime.", MessageType.Info);
					database.automaticLoad = EditorGUILayout.Toggle("Enable automatic loading in editor.", database.automaticLoad);
		...
		static void DrawDataInspector(bool _runtime)
		{
			...
			using (new GUILayout.VerticalScope("Box", database.showInspector ? GUILayout.Width(inspectorWidth) : GUILayout.Width(40)))
			{
				...
				if (!database.showInspector)
				{
					...
					#if UNITY_EDITOR
					if (GUILayout.Button(new GUIContent(database.collapsedView ? iconFullView : iconListView, database.collapsedView ? "Full view" : "List view"), GUILayout.Width(32), GUILayout.Height(32)))
					{
					...
					#endif
					#if UNITY_EDITOR
					database.showFieldTypes = GUILayout.Toggle(database.showFieldTypes, new GUIContent(iconDataType, "Show data types"), "Button", GUILayout.Width(32), GUILayout.Height(32));
					database.showInitValues = GUILayout.Toggle(database.showInitValues, new GUIContent(iconInitValue, "Show initial values"), "Button", GUILayout.Width(32), GUILayout.Height(32));
					#else
				...
				if (database.showInspector)
				{
					...
					if ( !_runtime )
					{
						using (new GUILayout.VerticalScope("Box"))
						{
							var _options = new string[]{"Add Value", "Add Scheme"};
						
							GUILayout.Space (3);
							selectedAddValueOption = EditorGUILayout.Popup(selectedAddValueOption, _options);
						
							// ADD VALUE
							if (selectedAddValueOption == 0)
							{
									...
									if (GUILayout.Button("Add value", GUILayout.Height(50)))
									{
										if (!addToSelected)
										{
											if (!string.IsNullOrEmpty(entryID) && !string.IsNullOrEmpty(valueID))
											{
												...
												System.Type _type = GetTypeFromString(allTypes[selectedTypeIndex]);
												//System.Type[] _type = System.AppDomain.CurrentDomain.GetAllDerivedTypes(allTypes[selectedTypeIndex]);
												var _instance = System.Activator.CreateInstance(_type);
										...
										else
										{
											if (!string.IsNullOrEmpty(valueID))
											{
												foreach (var entry in database.DB[selectedDatabaseTable].entries.Keys)
												{
													...
													{
														System.Type _type = GetTypeFromString(allTypes[selectedTypeIndex]);
														var _instance = System.Activator.CreateInstance(_type); //System.Type.GetType(allTypes[selectedTypeIndex]));
							...
							else
							{
								...
								updateScheme = GUILayout.Toggle(updateScheme, "Update selected entry");
								
								GUILayout.Label("Databox Scheme:");
								database.databoxScheme = (DataboxSchemes)EditorGUILayout.ObjectField(database.databoxScheme, typeof(DataboxSchemes), false);
								
								
								if (database.databoxScheme != null)
								{
									var _schemeNames = database.databoxScheme.schemes.Select(x => x.schemeName).ToList();
									...
									selectedScheme = EditorGUILayout.Popup(selectedScheme, _schemeNames.ToArray());
									...
									if (!updateScheme)
									{
										...
										if (GUILayout.Button("Add Scheme", GUILayout.Height(50)))
										{
											if (!string.IsNullOrEmpty(entryID))
											{
												for (int i = 0; i < database.databoxScheme.schemes[selectedScheme].schemeTypes.Count; i ++)
												{
													System.Type _type = GetTypeFromString(database.databoxScheme.schemes[selectedScheme].schemeTypes[i].type);
													var _instance = System.Activator.CreateInstance(_type);
													
													//DataboxType _defaultData = database.databoxScheme.schemes[selectedScheme].schemeTypes[i].defaultData;
			
													database.AddData(selectedDatabaseTable, entryID, database.databoxScheme.schemes[selectedScheme].schemeTypes[i].name, _instance as DataboxType);
												}
									...
									else
									{
										...
										if (GUILayout.Button("Update selected", GUILayout.Height(50)))
										{
											foreach (var entry in database.DB[selectedDatabaseTable].entries.Keys)
											{
												if (database.DB[selectedDatabaseTable].entries[entry].selected)
												{
													List<string> _newTypes = new List<string>();
													List<string> _newTypesNames = new List<string>();
													List<string> _removeTypes = new List<string>();
												
													// first check if theres a new entry in the scheme which doesn't exist in the selected entry
													for (int i = 0; i < database.databoxScheme.schemes[selectedScheme].schemeTypes.Count; i ++)
													{
														var _exists = false;	
														var _schemeTypeName = database.databoxScheme.schemes[selectedScheme].schemeTypes[i].name;
													
														foreach(var _valueName in database.DB[selectedDatabaseTable].entries[entry].data.Keys)
														{
														
															if (_schemeTypeName == _valueName)
															{
																_exists = true;
															}
														}
														...
														if (!_exists)
														{
															_newTypesNames.Add(_schemeTypeName);
															_newTypes.Add(database.databoxScheme.schemes[selectedScheme].schemeTypes[i].type);
															
														}
														
													}
													
													// second check if the entry has types which do not exist in the scheme
													foreach(var _valueName in database.DB[selectedDatabaseTable].entries[entry].data.Keys)
													{
													...
													//for (int n = 0; n < _newTypes.Count; n ++)
													//{
													//	Debug.Log("NEW TYPES: " + _newTypes[n]);
													//}
													
													//for (int n = 0; n < _removeTypes.Count; n ++)
													//{
													//	Debug.Log("REMOVE TYPES: " + _removeTypes[n]);
													//}
													
													var _removedTypes = string.Join(",", _removeTypes);
													
													if (_removeTypes.Count > 0 && EditorUtility.DisplayDialog("Warning",
														"Some types will be removed from selected entry:" + "\n" + _removedTypes + "\n" 																								
														+ "Are you sure you want to update?", "Yes", "No"))
													{
														// Add new types
														for (int n = 0; n < _newTypes.Count; n ++)
														{
															System.Type _type = GetTypeFromString(_newTypes[n]);
															var _instance = System.Activator.CreateInstance(_type);
			
															database.AddData(selectedDatabaseTable, entry, _newTypesNames[n], _instance as DataboxType);
														}
														
														// Remove types
														for (int r = 0; r < _removeTypes.Count; r ++)
														{
															database.RemoveValue(selectedDatabaseTable, entry, _removeTypes[r]);
														}
													}
		...
		static void DrawDataSheet()
		{
			...
			using (new GUILayout.VerticalScope())
			{
				...
				using (var scrollViewScope = new GUILayout.ScrollViewScope(scrollPosition, "Box"))
				{
					...
					if (!database.DB.ContainsKey(selectedDatabaseTable))
					{
						selectedDatabaseTable = database.DB.Keys.FirstOrDefault();
					}
						...
						if (_index < database.entriesPerPage * currentPage && _index >= (database.entriesPerPage * currentPage) - database.entriesPerPage)
						{
							...
							using (new GUILayout.VerticalScope(rowBackground, GUILayout.Height(29)))
							{
								...
								using (new GUILayout.HorizontalScope())
								{
									...
									{
										#if UNITY_EDITOR
										if (GUILayout.Button(new GUIContent(iconTrashIcon, "Remove entry"), GUILayout.Height(24), GUILayout.Width(40))) //GUILayout.Width(20), GUILayout.Height (20)))
										{
													...
													using (new GUILayout.HorizontalScope())
													{
														...
														using (new GUILayout.VerticalScope("Box"))
														{
															...
															using (new GUILayout.HorizontalScope("Toolbar"))
															{
																...
																{
																	#if UNITY_EDITOR
																	if (GUILayout.Button(EditorGUIUtility.IconContent("d_editicon.sml"), "ToolbarButton"))
																	{
																		valueRenameKey = dataKey;
																		newValueName = dataKey;
																	}
																	#endif
																	GUILayout.Label(dataKey);
																	//if (GUILayout.Button(dataKey + " : ", "ToolbarButton", GUILayout.Width(100)))
																	//{
																...
																else
																{
																	...
																	using (new GUILayout.HorizontalScope())
																	{
																		...
																		newValueName = GUILayout.TextField(newValueName, GUILayout.MinWidth(100));
																		...
																		if (GUILayout.Button("Ok", "ToolbarButton", GUILayout.Width(60)))
																		{
																		...
																		if (GUILayout.Button("Cancel", "ToolbarButton", GUILayout.Width(60)))
																		{
																			//valueRenamingActive = false;
																			renameValueError = false;
																			valueRenameKey  = "";
																...
																#if UNITY_EDITOR
																if (GUILayout.Button(new GUIContent(iconReset, "Reset value back to initial value"),  "toolbarButton"))
																#else
																...
																{
																#if UNITY_EDITOR
																	if (GUILayout.Button(new GUIContent(iconTrashIcon, "Remove value"), "toolbarButton", GUILayout.Width (40)))
																	{
		...
		static void LoadEditorResources()
		{
			...
			iconTrashIcon = (Texture2D)AssetDatabase.LoadAssetAtPath(_path + "/" + "trashCanIcon.png", typeof(Texture2D));
			iconReset = (Texture2D)AssetDatabase.LoadAssetAtPath(_path + "/" + "resetIcon.png", typeof(Texture2D));
		...
		public static void GetDataTypes(out List<string> _allTypes, out List<string> _typeNames)
		{
			...
			foreach(var type in types)
			{
				...
				{
					_typeNames.Add(type.FullName.ToString());
				}
				
				_allTypes.Add(type.FullName.ToString());
			}
		}

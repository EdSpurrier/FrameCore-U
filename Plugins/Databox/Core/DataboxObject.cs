...
namespace Databox
{
	...
	public class DataboxObject : ScriptableObject {
		...
		[SerializeField]
		public DataboxSchemes databoxScheme;
		...
		public object GetDataUnknown(string _tableID, string _entryID, string _valueID)
		{
			...
			if (DB[_tableID].entries[_entryID].data.TryGetValue(_valueID, out _temp))
			{
				if (_temp?.Count > 0)  
				{
					return _temp.FirstOrDefault().Value;
				}
				//for (int i = 0; i < DataboxEditor.allTypes.Count; i ++)
				//{
				//	_temp.TryGetValue(GetTypeFromString(DataboxEditor.allTypes[i]), out _data); //System.Type.GetType(DataboxEditor.allTypes[i]), out _data))
				//	if (_data != null)
				//	{
				//		return _data;
				//	}
		...
		/// <summary>
		/// Returns first entry of type T
		/// Only use it if theres just one entry in the database of type T
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public T GetDataOfType<T>()where T : DataboxType
		{
			DataboxType _dataType;
			
			foreach (var table in DB.Keys)
			{
				foreach (var entry in DB[table].entries.Keys)
				{
					foreach(var value in DB[table].entries[entry].data.Keys)
					{
						if(DB[table].entries[entry].data[value].TryGetValue(typeof(T), out _dataType))
						{
							return (T)_dataType;
						}
					}
		...
		private System.Type GetTypeFromString(string _typeName)
		{
		...
		public void ResetToInitValues(string _table)
		{
			foreach(var entry in DB[_table].entries.Keys)
			{
				foreach (var data in DB[_table].entries[entry].data.Keys)
				{
					...
					{
						foreach(var d in _tempData.Values)                            
							d.Reset();
						//for (int i = 0; i < DataboxEditor.allTypes.Count; i ++)
						//{
						//	DataboxType _data;
						//	_tempData.TryGetValue(GetTypeFromString(DataboxEditor.allTypes[i]), out _data); //System.Type.GetType(DataboxEditor.allTypes[i]), out _data))
						//	if (_data != null)
						//	{
						//		_data.Reset();
						//	}
